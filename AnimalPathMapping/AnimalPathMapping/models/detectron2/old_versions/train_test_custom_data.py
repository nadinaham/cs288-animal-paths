...
def get_custom_dicts(ids, img_dir, mask_dir):
    ...
    # image_files = [f for f in os.listdir(img_dir) if f.startswith('image') and f.endswith('.png') and f.split('-')[-1].split('.')[0] in ids] # TODO take out bc images don't start with 'image'
...
# print(f"os list dir img_dir: {os.listdir(img_dir)}")
# print(f"all image ids: {all_image_ids}")
...
for idx, d in enumerate(sampled_dicts):
    ...
    v = Visualizer(im[:, :, ::-1], metadata=custom_metadata, scale=0.8) # originally had scale=0.8, should not have < 1 as paths will be too thin for proper outputs
...
evaluator = COCOEvaluator("custom_test", ("segm",), False, output_dir=f"{output_dir}/inference")
val_loader = build_detection_test_loader(cfg, "custom_test")
results = inference_on_dataset(trainer.model, val_loader, evaluator)
print(results) # results will be in the bash output file if called with sbatch
